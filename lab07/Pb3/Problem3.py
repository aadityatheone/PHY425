...
def CNM(xi,xf,ti,tf,g1,g2,nx,nt):
    h=(xf-xi)/nx
    k=(tf-ti)/nt
    l=k/h**2
    print h,k
    ...
    for j in range(2):
        ...
        for i in range(nx-1):
            u[j+1][i+1]=un[i]
    for j in range(2,nt):
        u[j+1][0]=(1-2*h*l-2*l)*u[j][0]+2*l*u[j][1]
        u[j+1][nx]=2*l*u[j][nx-1]+(1-2*l)*u[j][nx]
        for i in range(1,nx):
            u[j+1][i]=u[j-1][i]+2*l*(u[j][i-1]-2*u[j][i]+u[j][i+1])
    return x,t,u,h,k,l

xi,xf,ti,tf=0.,4.,0.,10./36.
nt,nx=10,24
g1,g2=0.,0.
x,y,u,h,k,l=CNM(xi,xf,ti,tf,g1,g2,nx,nt)
np.savetxt('Result3.txt',u,fmt='%.2f')
plt.pcolor(x,y,u,cmap='jet')
plt.colorbar().set_label(r'$u(x,t)$', fontsize=12)
plt.xlabel(r'$x$')
plt.ylabel(r'$t$')
plt.title(r' $h='+str(h)+'$'+r', $k='+str(k)+'$'+r', $\lambda='+str(l)+'$')
plt.suptitle(r'Problem 3: Crank-Nicolson method with derivative B.C.')
